...
class VAE_Encoder(nn.Sequential):
    def __init__(self):
        super().__init__(
            # (Batch, 3, Height, Width) -> (Batch, 128, Height, Width)
            nn.Conv2d(3, 128, kernel_size=3, padding=1),
            
            # (Batch, 128, Height, Width) -> (Batch, 128, Height, Width)
            VAE_ResidualBlock(128, 128),
            # FIX: checkpoint expects two ResidualBlocks at 128->128 before downsampling
            VAE_ResidualBlock(128, 128),
    ...
    def forward(self, x: torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        ...
        for module in self:
            if getattr(module, 'stride', None) == (2, 2):
                # Asymmetric padding for stride-2 convolutions
                x = F.pad(x, (0, 1, 0, 1))
            x = module(x)
        ...
        # Sample from the latent distribution: z = mean + stdev * noise
        x = mean + stdev * noise
